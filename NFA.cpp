...
// DESC: Given a single operand a 2 state NFA is created (start = 0, final = 1).
...
NFA::NFA(char operand) {
    ...
    // set this NFA's size to 2 (2 states: 0 and 1)
    _delta.resize(2, vector<pair<int, char>> {});
    // add a transition to state 0 to state 1 with the operand
...
//       In the case of union or concatenation. A new NFA is made my combining
...
NFA::NFA(NFA M1, char op, NFA M2) {     // M2 has a default value to make it an optional parameter
    ...
    // UNION - create new start which will have a transition to the start states of m1 and m2
    if (op == '+') {
        ...
        for (int i = 0; i < M1.getSize(); i++) {
            vector<pair<int, char>> temp = M1.getDelta()[i];    // get vector of transition pairs from M1 state i
            ...
            _delta.push_back(temp);     // push modified vector of M1 pairs
        ...
        for (int i = 0; i < M2.getSize(); i++) {
            vector<pair<int, char>> temp = M2.getDelta()[i];    // get vector of transition pairs from M2 state i

            // increment each state num in vector of pairs by size of M1 + 1
            for (int j = 0; j < temp.size(); j++) {
    ...
    // CONCATENATION - direct the final states of m1 to the start state of m2. The only final states will be from m2
    else if (op == '.') {
        ...
        for (int i = 0; i < M2.getSize(); i++) {
            vector<pair<int, char>> temp = M2.getDelta()[i];    // get vector of transition pairs from M2 for state i
    ...
    // KLEEN STAR - Give final states a transition back to the start state, and the start state to each final state
    else if (op == '*') {
    ...
    // After accommodating the operator remove Epsilons from newly made NFA
    removeEpsilon();
}
...
void NFA::removeEpsilon() {
    ...
    // iterate the current NFA's transitions (2d vector)
    for (int i = 0; i < _delta.size(); i++) {
        for (int j = _delta[i].size() - 1; j >= 0; j--) {
            bool restart = false;   // used if a new & transition is added

            // if state i has an epsilon transition (&)
            if (get<1>(_delta[i][j]) == '&') {
                //source = state = i
                //initialize destination (dst) and get it from the current pair
                int dst = get<0>(_delta[i][j]);

                _delta[i].erase(_delta[i].begin() + j); // delete epsilon (&) transition

                // Give the source state all the transitions the destination state has
                for (int k = 0; k < _delta[dst].size(); k++) {

                    // don't enter if statement if it is an epsilon transition with a destination of the current source state
                ...
                // in case destination is a final state make source state a final state as well
...
NFA convert(string w) {
    return get<0>(convertHelper(0, w)); // get the NFA from the pair used in the recursive helper function
}
